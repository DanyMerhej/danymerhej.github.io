...
export interface Project {
  ...
  highlights: string[];
  /** The interesting engineering problem — what a peer would actually want to read. */
  engineering: string;
  stack: string[];
  links: ProjectLink[];
...
export const metrics = [
  { value: 7, suffix: '+', label: 'years building software', sub: 'intern → team lead, one company' },
...
export const impactStats = [
  ...
  { value: '25%', label: 'faster delivery', detail: 'after introducing Kanban + Jira agile flow' },
...
export const experience: Role[] = [
  {
    ...
    place: 'Jdeideh, Lebanon',
    current: true,
    tags: ['Leadership', 'Architecture', 'InsurTech', 'Client-facing'],
    points: [
      'Lead a team of 2–3 developers across the architecture, development and delivery of IRIS — policy administration, underwriting, claims, accounting, collections, reporting, reinsurance, broker portals and third-party integrations — used by 30+ insurers across the Middle East, Africa and Europe.',
      ...
      'Introduced agile delivery (Kanban, Jira), improving delivery speed and project turnaround by 25%.',
      'Led performance tuning and database optimisation across systems holding 1TB+ of data, cutting query execution time by 30%.',
      'Mentor junior and mid-level developers on architecture and engineering practice — team productivity up 20%.',
    ],
  },
  {
    company: 'Pixel Software Solutions',
    ...
    place: 'Beirut Governorate, Lebanon',
    tags: ['C#', 'VB.NET', 'SQL Server', 'Solution design'],
    points: [
      "Owned end-to-end delivery for enterprise insurance clients — requirements, solution design, development, release and maintenance — across IRIS's policy administration, underwriting and claims modules.",
  ...
  },
  {
    ...
    place: 'Jdeideh, Lebanon',
    points: [
      'First exposure to .NET and enterprise insurance software — the start of a 7+ year run at the same company.',
    ],
  },
  {
    company: 'BESTSELLER',
    ...
    place: 'ABC Dbayeh, Lebanon',
    tags: ['Before engineering'],
    points: [
      'Progressed Sales Executive → Cashier → Visual Merchandising Coordinator → Assistant Store Manager.',
...
export const projects: Project[] = [
  {
    ...
    blurb:
      'Photograph a receipt, get it itemised by AI, split it with anyone — live, offline-capable, multi-currency.',
    status: 'live',
    year: '2024 — now',
    ...
    hues: ['#C6F94E', '#39D0A5'],
    highlights: [
      'AI receipt scanning — Gemini extracts items, prices and totals from a photo, no manual entry',
      'Live sessions: several people split the same bill in real time from their own phones',
      'Multi-currency USD ⇄ LBP side by side with automatic exchange-rate detection',
      'Offline-first via IndexedDB — the app keeps working with no connection',
      'Friends, groups, trips, settle-up balances and full transaction history',
      'Analytics dashboard with CSV and PDF export',
      'One codebase → web (Cloudflare Pages) and Android (Capacitor, Google Play)',
    ],
    engineering:
      'The hard part was never the splitting maths — it was consistency. Receipts arrive as images and have to become structured data reliably enough to trust, so the Gemini pipeline runs server-side in a Deno edge function with a strict schema and fallbacks for the shapes real receipts take (tax after the subtotal, service charges, mixed currencies). On top of that sits an offline-first data model: an IndexedDB cache that has to reconcile with Supabase/Postgres when several devices edit the same split at once. Auth is Google OAuth, the whole backend is serverless, and the same TypeScript codebase ships to the browser and to the Play Store.',
    stack: [
      'React 18',
      'TypeScript',
      'Vite',
    ...
    links: [
      ...
      { label: 'Google Play', href: 'https://play.google.com/store/search?q=splittyy&c=apps', kind: 'store' },
    ],
  },
  {
    id: 'eventyy',
    ...
    blurb:
      'A multi-tenant event operations platform. Any event type, modular by design — never hardcoded to weddings.',
    status: 'building',
    year: '2025 — now',
    ...
    highlights: [
      ...
      'Event-type-driven: weddings, birthdays, funerals, corporate events are peers, configured as data',
      'Modules toggle per event and per plan — suppliers, finance, media, tasks, timeline, discussions, knowledge',
      'Smart Notes instead of plain notes: reminders, attachments, voice, mentions, AI actions',
      'Quotations, contracts and payments with printable, generated finance documents',
      'Client and supplier portals with per-event, module-scoped access',
      'Trilingual from day one — English, Arabic, French — with full RTL',
    ],
    engineering:
      'This one is an exercise in getting the boundaries right before writing features. Tenant isolation is enforced in Postgres, not in the UI: every tenant table carries a non-null org_id, ships with its RLS policies, and lands in the same commit as a pgTAP test proving one organisation cannot read another. Two security-definer helpers describe the entire access model — one for org membership, one for per-event external participants. AI is a layer, not a feature: everything runs through a single gateway edge function so today\'s provider can be swapped without touching product code. Storage uses path-based RLS, and the build order is enforced by a written spec so no module ships without schema, policies, screens, mobile parity, realtime and isolation tests.',
    stack: [
      'React 18',
      'TypeScript (strict)',
      'Vite',
    ...
    links: [{ label: 'eventyy.co', href: 'https://eventyy.co', kind: 'site' }],
  },
  {
    id: 'beautyflow',
    name: 'BeautyFlow',
    tagline: 'Salon & clinic operations, done properly',
    blurb:
    ...
    highlights: [
      ...
      'Appointments — day/week views, drag-to-reschedule, multi-service and multi-staff bookings, conflict detection',
      'Status pipeline: New → Confirmed → Checked In → In Service → Completed / No Show / Cancelled',
      'Customer CRM with a unified timeline and computed insights — lifetime value, visit frequency, churn risk',
      'POS with mixed payments (cash, card, transfer, wallet), discounts and a cash drawer',
      'Finance that shows real profit: revenue − COGS − commissions − operating costs, including recurring costs like a daily-rented laser machine',
      'Nothing about the business domain is hardcoded — services, categories, costs, roles are all editable data',
    ],
    engineering:
      'Most salon software is either a pretty calendar with no accounting, or an ERP nobody wants to open. BeautyFlow tries to be neither. The scheduling core does real conflict detection across staff, rooms and equipment while staying drag-and-drop fluid on a phone, and the finance layer computes genuine profitability rather than revenue — which means modelling cost of goods, per-staff commission and fixed recurring costs as first-class data. Everything is domain-agnostic: a nail bar, a laser clinic and a wellness centre run the same build with different rows in the database. One codebase ships to web, iOS and Android via Capacitor, with Playwright covering the end-to-end paths.',
    stack: [
      'React 18',
      'TypeScript (strict)',
      'Vite',
  ...
  },
  {
    ...
    hues: ['#FFD166', '#FF7A45'],
    highlights: [
      '15 level-gated businesses with payback times from seconds to days',
      ...
      'Story chapters, daily/weekly missions, achievements, streaks and offline earnings',
      'Economy tuned by simulation — millionaire on day one, trillionaire only after prestige',
    ],
    engineering:
      'An idle game is a balance problem wearing a game costume. The interesting work is the economy: fifteen assets whose costs and payouts have to stay meaningful across fifteen orders of magnitude, tuned by running simulations rather than guessing, plus offline earnings and rivals that advance correctly across a closed app. State is a single Zustand store persisted to AsyncStorage with migration-safe schemas, animation runs on Reanimated worklets so the numbers never stutter, and monetisation is deliberately opt-in — rewarded boosts and cosmetics, never pay-to-win.',
    stack: [
      'Expo 57',
      'React Native 0.86',
      'React 19',
  ...
  },
  {
    ...
    blurb:
      'A bespoke dark, high-contrast commerce theme for a gym and supplements audience, trading today.',
    status: 'live',
    year: '2025',
    role: 'Design & build',
    glyph: '⬢',
    hues: ['#FF4D4D', '#FFB020'],
    highlights: [
      'Custom "Alpha" design layer built on Shopify Dawn 15.5 as a foundation',
      ...
      'Two-way GitHub ⇄ Shopify sync — pushes to main deploy, theme-editor edits commit back',
      'Local development against live store data with hot reload, nothing written to production',
    ],
    engineering:
      'The constraint here is maintenance, not looks. Forking a Shopify theme and editing it freely is how stores end up permanently stranded on an old version, so the bespoke layer sits deliberately apart from Dawn\'s internals — Dawn can be upgraded without unpicking the design. Deployment has no CI and no secrets: Shopify pulls from the repository directly, and the fact that theme-editor changes commit back to main is treated as expected behaviour rather than a conflict to fight.',
  ...
  },
  {
    ...
    blurb:
      'A luxury watch storefront where every listing is a story — provenance, condition report, calibre, era.',
    status: 'building',
    year: '2025 — now',
    role: 'Design & build',
    glyph: '◷',
    hues: ['#C9A227', '#8E7B3F'],
    highlights: [
      'An "Aged Patina" luxury design system on Shopify Craft',
      'Editorial homepage with featured timepieces, brand story and journal',
      'Collector-grade product pages: reference, era, movement, case diameter, lug width',
      'Full condition report — dial, case, crown, crystal, movement, strap — plus box & papers',
      'Historical background, restoration details and provenance per watch',
      'Brand taxonomy mega-menu, faceted filtering, SEO and structured data',
    ],
    engineering:
      'Selling a vintage watch is selling trust, so the product page had to carry far more than a price and a photo. Every collector-grade field — reference, calibre, condition per component, restoration history, provenance — is a Shopify metafield the template reads and hides gracefully when empty, which means the merchant can list a watch with three facts or thirty and the page stays composed either way. Brand comes from the vendor field so the eyebrow, the cards and the mega-menu taxonomy all stay in sync from one source.',
...
export const capabilities: Capability[] = [
  ...
  },
  {
    ...
    glyph: '◍',
    body:
      'AI as an engineered layer, not a bolt-on demo — provider abstractions, structured extraction, and a fallback for every failure mode.',
    points: [
      'Vision → structured data pipelines',
  ...
  },
  {
    ...
    glyph: '◑',
    body:
      'Products that feel like Linear or Stripe, not like enterprise software. Responsive, accessible, internationalised, RTL-ready.',
    points: [
